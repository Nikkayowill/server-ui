...
exports.showDashboard = async (req, res) => {
    try {
        ...
        const dashboardHTML = buildDashboardTemplate({
            ...
            siteLimit,
            trialAvailable,
            autoDeployEnabled: server?.auto_deploy_enabled || false,
        ...
        <div id="domains" class="bg-gray-800 rounded-lg p-6 scroll-mt-24">
            ...
            ${data.domains.length > 0 ? `
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <div class="bg-black bg-opacity-30 border border-gray-700 rounded-lg p-4" data-domain-id="${dom.id}">
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-3">
                            <div class="flex items-center gap-3">
                                <p class="text-sm text-white font-bold">${escapeHtml(dom.domain)}</p>
                                ${dom.ssl_enabled 
                                    ? '<span class="px-2 py-1 text-xs font-bold uppercase rounded bg-green-900 text-green-300">🔒 SSL</span>' 
                                    : '<span class="px-2 py-1 text-xs font-bold uppercase rounded bg-yellow-900 text-yellow-300">⏳ SSL</span>'
                                }
                                ${dom.deployment_status === 'deployed'
                                    ? '<span class="px-2 py-1 text-xs font-bold uppercase rounded bg-green-900 text-green-400">✓ Deployed</span>'
                                    : dom.deployment_status === 'deploying'
                                    ? '<span class="px-2 py-1 text-xs font-bold uppercase rounded bg-blue-900 text-blue-300">⏳ Deploying</span>'
                                    : dom.deployment_status === 'failed'
                                    ? '<span class="px-2 py-1 text-xs font-bold uppercase rounded bg-red-900 text-red-300">✗ Failed</span>'
                                    : dom.git_url 
                                    ? '<span class="px-2 py-1 text-xs font-bold uppercase rounded bg-gray-700 text-gray-300">Linked</span>'
                                    : ''
                                }
                            </div>
                            <div class="flex items-center gap-2">
                                ${dom.ssl_enabled && dom.deployment_status === 'deployed' ? `
                                <a href="https://${escapeHtml(dom.domain)}" target="_blank" class="px-3 py-1 bg-brand hover:bg-cyan-600 text-black text-xs font-bold rounded transition-colors">Visit ↗</a>
                                ` : ''}
                                ${dom.git_url ? `
                                <button onclick="deployDomain(${dom.id}, '${escapeHtml(dom.domain).replace(/'/g, "\\'")}')" class="px-3 py-1 bg-green-600 hover:bg-green-700 text-white text-xs font-bold rounded transition-colors">Deploy</button>
                                ` : ''}
                                <button onclick="openLinkRepoModal(${dom.id}, '${escapeHtml(dom.domain).replace(/'/g, "\\'")}', '${escapeHtml(dom.git_url || '').replace(/'/g, "\\'")}')" class="px-3 py-1 bg-gray-600 hover:bg-gray-500 text-white text-xs font-bold rounded transition-colors">${dom.git_url ? 'Edit Repo' : 'Link Repo'}</button>
                                <button onclick="deleteDomain(${dom.id}, '${escapeHtml(dom.domain).replace(/'/g, "\\'")}')" class="px-3 py-1 bg-red-600 hover:bg-red-700 text-white text-xs font-bold rounded transition-colors">Delete</button>
                            </div>
                        </div>
                        
                        ${dom.git_url ? `
                        <div class="bg-gray-900 rounded p-2 flex items-center justify-between">
                            <code class="text-xs text-gray-400 font-mono truncate flex-1">${escapeHtml(dom.git_url)}</code>
                            <div class="flex items-center gap-2 ml-2">
                                ${dom.auto_deploy_enabled 
                                    ? `<span class="px-2 py-1 text-xs font-bold uppercase rounded bg-purple-900 text-purple-300">🔄 Auto</span>
                                       <button onclick="disableDomainAutoDeploy(${dom.id})" class="text-xs text-red-400 hover:text-red-300">Disable</button>`
                                    : `<button onclick="enableDomainAutoDeploy(${dom.id})" class="text-xs text-purple-400 hover:text-purple-300">Enable Auto-Deploy</button>`
                                }
                            </div>
                        </div>
                        ${dom.last_deployed_at ? `<p class="text-xs text-gray-500 mt-2">Last deployed: ${new Date(dom.last_deployed_at).toLocaleString()}</p>` : ''}
                        ` : `
                        <p class="text-xs text-gray-500">Link a git repository to deploy to this domain</p>
                        `}
                    </div>
                    `).join('')}
                </div>
...
function openLinkRepoModal(domainId, domainName, currentGitUrl) {
    currentDomainId = domainId;
    currentDomainName = domainName;
    document.getElementById('linkRepoModal').classList.remove('hidden');
    document.getElementById('linkRepoModal').classList.add('flex');
    document.getElementById('linkRepoDomain').textContent = domainName;
    document.getElementById('domainGitUrl').value = currentGitUrl || '';
}

function closeLinkRepoModal() {
    document.getElementById('linkRepoModal').classList.remove('flex');
    document.getElementById('linkRepoModal').classList.add('hidden');
    currentDomainId = null;
    currentDomainName = null;
}

async function linkDomainRepo() {
    const gitUrl = document.getElementById('domainGitUrl').value.trim();
    if (!gitUrl) {
        alert('Please enter a Git URL');
        return;
    }
    if (!gitUrl.endsWith('.git')) {
        alert('Git URL must end with .git');
        return;
    }
    
    try {
        const res = await fetch('/link-domain-repo', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'x-csrf-token': '${data.csrfToken}' },
            body: JSON.stringify({ domainId: currentDomainId, gitUrl })
        });
        const result = await res.json();
        if (result.success) {
            closeLinkRepoModal();
...
async function deployDomain(domainId, domainName) {
    if (!confirm('Deploy to ' + domainName + '?')) return;
    
    try {
        const res = await fetch('/deploy-domain', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'x-csrf-token': '${data.csrfToken}' },
        ...
        const result = await res.json();
        if (result.success) {
            alert('Deployment started for ' + domainName + '! Refresh to see progress.');
            window.location.reload();
        } else {
            alert('Error: ' + result.error);
        }
...
async function enableDomainAutoDeploy(domainId) {
    try {
        const res = await fetch('/enable-domain-auto-deploy', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'x-csrf-token': '${data.csrfToken}' },
            body: JSON.stringify({ domainId })
        });
        const result = await res.json();
        if (result.success) {
            alert('Auto-deploy enabled!\\n\\nWebhook URL: ' + result.webhookUrl + '\\n\\nAdd this to your GitHub repo settings.');
            window.location.reload();
        } else {
            alert('Error: ' + result.error);
        }
    } catch (err) {
        alert('Failed to enable auto-deploy');
    }
}

async function disableDomainAutoDeploy(domainId) {
    if (!confirm('Disable auto-deploy for this domain?')) return;
    
    try {
        const res = await fetch('/disable-domain-auto-deploy', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'x-csrf-token': '${data.csrfToken}' },
            body: JSON.stringify({ domainId })
        });
        ...
        alert('Failed to disable auto-deploy');
    }
}

function deleteDomain(domainId, domainName) {
    if (!confirm('Delete domain ' + domainName + '?\\n\\nThis will also remove any deployments linked to this domain.')) return;
    
    // Create and submit form
    const form = document.createElement('form');
    form.method = 'POST';
    form.action = '/delete-domain';
...
<div id="linkRepoModal" class="hidden fixed inset-0 bg-black bg-opacity-80 z-50 justify-center items-center">
    <div class="bg-gray-900 border border-gray-700 rounded-lg p-6 max-w-lg w-11/12">
        <h2 class="text-xl font-bold text-white mb-2">Link Repository</h2>
        <p class="text-sm text-gray-400 mb-4">Link a git repository to <span id="linkRepoDomain" class="text-brand font-bold"></span></p>
        
        <div class="mb-4">
            <label class="block text-xs font-bold text-gray-400 uppercase mb-2">Git Repository URL</label>
            <input type="url" id="domainGitUrl" placeholder="https://github.com/user/repo.git" class="w-full px-4 py-3 bg-gray-800 border border-gray-600 rounded-lg text-white focus:border-brand focus:outline-none">
            <p class="text-xs text-gray-500 mt-1">Must be a public HTTPS URL ending in .git</p>
        </div>
        
        <div class="flex justify-end gap-3">
            <button onclick="closeLinkRepoModal()" class="px-4 py-2 bg-gray-700 text-white rounded hover:bg-gray-600 transition-colors">Cancel</button>
            <button onclick="linkDomainRepo()" class="px-4 py-2 bg-blue-600 text-white font-bold rounded hover:bg-blue-700 transition-colors">Link Repository</button>
        </div>
    </div>
</div>
  `;
